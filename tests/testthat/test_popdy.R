...
param <- list(r_max = 0.2,
              K = 1000,
              z = 2.39)
data0 <- list(catch = data.frame(year = tspan[1]:tspan[2],
                                 catch = rep(0, num_years)))
data_sust <- list(catch = data.frame(year = tspan[1]:tspan[2],
                                     catch = rep(1 - 0.6 * param$K, num_years)))
...
test_that("Gen. logistic with catches", {
  tspan <- c(1990, 2000)
  data <- list(catch = data.frame(year = 1990:1999,
                                  catch = c(10, 20, 40, 80, 160,
                                            320, 40, 40, 20, 10)))
  P_catches <- project_population(param_sample = param,
                                  data = data,
                                  tspan = tspan)
  Pmin_catches <- minimum_population(P_catches)
  ## Predicted values generated 2018-08-19; testing this will only
  ## tell us if values change
  pred <- c(1000, 990, 974.69935403084, 946.280493711608,
            889.67821393339, 773.049600240948, 524.089030824881,
            566.52925365989, 610.697507599572, 675.254944423984,
            747.470442533935)
...
test_that("Catch series checks work", {
  data <- list(catch = data.frame(year = 1991:2000,
                                  catch = rep(2, 10)))
  expect_error(project_population(param, data, tspan),
               "must start the same year")
  data <- list(catch = data.frame(year = 1990:1996,
                                  catch = rep(2, 7)))
  expect_error(project_population(param, data, tspan),
               "must cover all but the final year")
})
